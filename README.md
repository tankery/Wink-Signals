...
The original library is a header-only library (mainly because of templates), therefore there is no installation. Just place it in your project, or somewhere your compiler can find it and you're all set!

But if you need to use the additional cross-thread signal-slot feature, you should build the source as library.

The project is generated by [GYP](https://code.google.com/p/gyp/). Install gyp, then run `./gyp-config/build/build_{android,ios,test}` to generate the specific platform project. Or run `./gyp-config/build/build_clean` to clear the project files.
...
Use `./gyp-config/build/build_test` to generate build system for your platform. Then make it use your faverate toolchain.
